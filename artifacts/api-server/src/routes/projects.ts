...
router.get("/", async (req, res) => {
  ...
  const conditions: SQL[] = [eq(projectsTable.userId, uid)];
...
router.post("/", async (req, res) => {
  ...
  const [project] = await db.insert(projectsTable).values({ ...parsed.data, userId: req.user.id }).returning();

  await db.insert(timelineEventsTable).values({
    projectId: project!.id, type: "projeto_criado", title: "Projeto criado",
    description: `Projeto "${project!.name}" foi criado`,
  });

  res.status(201).json({
    id: project!.id, name: project!.name, clientName: project!.clientName, type: project!.type,
    status: project!.status, priority: project!.priority, description: project!.description,
    aiSummary: project!.aiSummary, progress: project!.progress,
...
router.get("/:id", async (req, res) => {
  ...
  const [tasks, files, timeline, projectPeople] = await Promise.all([
    ...
    db.select({ person: peopleTable }).from(projectPeopleTable)
  ...
  res.json({
    ...
    timeline: timeline.map((e) => ({
      id: e.id, projectId: e.projectId, type: e.type, title: e.title,
...
const updateSchema = z.object({
  name: z.string().optional(), clientName: z.string().optional(), type: z.string().optional(),
  status: z.string().optional(), priority: z.string().optional(),
  description: z.string().optional(), progress: z.number().optional(),
});
...
router.patch("/:id", async (req, res) => {
  ...
  if (parsed.data.status) {
    await db.insert(timelineEventsTable).values({
      projectId: id, type: "status_atualizado", title: "Status atualizado",
